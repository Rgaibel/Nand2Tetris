...
import static auxPack.Grammar.FIELD_KEYWORD;
import static auxPack.Grammar.STATIC_KEYWORD;
...
public class JackTokenizer {
    ...
    public JackTokenizer(String filePath) throws IOException {
        ...
        boolean passLine = false; //indicates which lines to pass over
    ...
    private void splitToToken(String word, List<String> wordTokens) {
        ...
        for (int i = 0; i < symbols.size(); i++) {//iterate through symbols in Grammar symbol list
    ...
    //returns token (seems unnecessary)
    public String getToken() {
        return token;
    }
    //returns filename (never used?)
    public String getFileName() {
        return fileName;
    }
    //returns file path (never used?)
